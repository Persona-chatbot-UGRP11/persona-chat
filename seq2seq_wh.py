...
def readText():
    ...
    inputs = []
    outputs = []
    dialog = open('../input/chat201025/chat_.txt', encoding='utf-8').read().strip().split('\n')
...
def readText():
    ...
    inputs = []
    outputs = []
    dialog = open('./Seq2seq_data/chat_dnk.txt', encoding='utf-8').read().strip().split('\n')
    for i in range(len(dialog)-1):
        buff = dialog[i+1]
        buf = buff.split("\t")
        inputs.append(buf[0])
...
def indexesFromSentence(lang, sentence):
    ...
    for word in tokenizer.morphs(sentence):
        a = length
        for w, index in lang.word2index.items():  
            if w == word:
                lst.append(lang.word2index[word])
                length += 1
        if length == a:
            print('unk: ',word)
            lst.append(2) #unk_token
    return lst
...
#yg modify)
#문장생성
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH): #sentence == input pair
    with torch.no_grad():
        ...
        for di in range(max_length):
            ...
            #print(topi)
            #print('item(): ',topi.item())
            if topi.item() == EOS_token:
                decoded_words.append('<EOS>')
                break
            else:
                if topi.item() == UNKNOWN_token:
...
def evaluateinput(encoder, decoder, n=10):
    for i in range(n):
        text = input()
        print('>', text)
