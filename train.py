...
def chat(model_params, sent='0'):
    ...
    #while 1:
        #q = input('user > ').strip()
    with open('chat_test_question/office_input_question.txt', 'rt', encoding = 'UTF-8') as data:
        origin = data.read()
...
def seq2seq_evaluate(text_list):
    #load model
    #(dnk model)
    save_encoder = EncoderRNN(6505,256)
    save_decoder = AttnDecoderRNN(256, 6251)

    encoder_file = open("seq2seq_encoder.pkl", "rb")
    save_encoder = pickle.load(encoder_file)
    encoder_file.close()

    decoder_file = open("seq2seq_decoder.pkl", "rb")
...
if __name__ == "__main__":
    ...
    if opt.chat:
        ...
        final_output_list = seq2seq_evaluate(gpt_output_list)
        
        with open('gpt_s2s_result.txt', 'wt', encoding = 'UTF-8') as f:
